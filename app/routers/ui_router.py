...
def home(request: Request):
    try:
        ...
        )  # request means we are passingt the templates  home.html means which html file ,request:Request means jinja2 needs it
...
def ui_feeds(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "feeds.html", {"request": request, "feeds": feeds}
    )

...
from app.schemas.feed_schema import \
    FeedCreate  # class import for validation from table's pydantic class
...
def fetch_and_save_items(feed_id: int, db: Session = Depends(get_db)):
    ...
    print(items)
    return {"feed_id": feed.id, "saved_items": len(items)}
...
def get_items_from_db(feed_id: int, db: Session = Depends(get_db)):
    ...
    return {"feed_id": feed_id, "total_items": len(items), "items": items}
...
def mark_item_read(item_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(item)
    return {"message": "item marked as read", "item_id": item.id}
...
def get_unread_items(feed_id: int, db: Session = Depends(get_db)):
    ...
    unread_items = (
        db.query(Item).filter(Item.feed_id == feed_id, Item.is_read == False).all()
    )
...
def search_items(q: str, db: Session = Depends(get_db)):
    ...
    results = (
        db.query(Item).filter(func.to_tsvector("english", Item.title).match(q)).all()
    )
